...
class ServiceMain
{
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			LOG.info("Instantiating Dymola service proxy");
			simImpl = new SimulatorServerImpl(serviceWorkDir, dymolaStartPort,
					dymolaPortsRangeSize, dymolaDebug);
			...
			LOG.info("Publishing dymola service, url is: " + serviceURL);
			LOG.info(" thread pool size: " + wsNthreads);
			dymolaService.endpoint.publish(serviceURL);
		}
		catch (Exception e)
		{
			LOG.error("Error starting Dymola service. {}", e.getMessage());
			if (simImpl != null)
			{
				simImpl.close();
			}
