...
public class StandaloneDymolaClient
{
	...
	protected boolean prepareDynamicEnvironment(Path workingDirectory) {
		...
		while (retry.shouldRetry())
		{
			try
			{
				...
				if(Boolean.parseBoolean((String) sdh.getContent()) == true) {
					retCode = true;
					LOGGER.info("Dymola - Preparing workspace ended successfully");
					sdh.close();
					break;
				} else {
					sdh.close();
	...
	protected boolean check(Path workingDirectory, String modelName, String modelFileName, String outputDymolaFileName)
			throws Exception
	{
		...
		while (retry.shouldRetry())
		{
			try
			{
				...
				if(Boolean.parseBoolean((String) sdh.getContent()) == true) {
					retCode = true;
					LOGGER.info("Dymola - Check ended successfully");
					sdh.close();
					break;
				} else {
					sdh.close();
			...
			throws Exception
	{
		...
		while (retry.shouldRetry())
		{
			try
			{
				...
				StreamingDataHandler sdh = (StreamingDataHandler) dhout;
				LOGGER.info(
						"Dymola - remote dymola proxy service ended successfully, retrieving simulation output");
				
				sdh.moveTo(pathOut.toFile());
				sdh.close();
				LOGGER.info("Dymola - simulation output retrieved");
				break;
			}
			catch (Exception e)
			{
