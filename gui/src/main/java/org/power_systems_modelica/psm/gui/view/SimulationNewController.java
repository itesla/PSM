...
public class SimulationNewController extends MainChildrenController
{
	@Override
	...
	private void handleAddEvent()
	{
		...
		if (actionEvent.getSelectionModel().getSelectedItem().equals("BusFault"))
		{
			...
			double Z = Math.pow(Double.parseDouble(R.getValue()), 2)
					+ Math.pow(Double.parseDouble(X.getValue()), 2);
			if (Z < 1e-6)
			{
				UtilsFX.showWarning("Warning",
						"Impedance parameter is too low, the minimum value is 0.001.\nPlease insert different values for R or X.");
				return;
			}
		}
	...
	private void startWorkflow(boolean onlyCheck, boolean onlyVerify)
	{
		...
		try {
		    Integer.parseInt(stepBySecond);
		} catch (NumberFormatException e) {
			UtilsFX.showWarning("Warning", "The number of intervals per second should be an integer value.\nPlease insert different value for Steps per second.");
			return;
		}

		if(Double.parseDouble(stopTime) * Integer.parseInt(stepBySecond) <= 1.0) {
			UtilsFX.showWarning("Warning", "The number of intervals should be grater than 1.\nPlease insert different values for Stop time or Steps per second.");
			return;
		}
