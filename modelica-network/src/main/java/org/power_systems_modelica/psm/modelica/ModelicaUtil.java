...
public class ModelicaUtil
{
	...
	public static Map<String, ModelicaModel> groupByNormalizedStaticId(ModelicaDocument mo)
	{
		...
		Map<String, List<BaseModelicaDeclaration>> ds = mo.getSystemModel()
				.getDeclarations()
				.stream()
		...
		Map<String, List<AnnotationItem>> as = mo.getSystemModel().getAnnotation().getItems()
				.stream()
	...
	public static String getNormalizedStaticId(BaseModelicaDeclaration d)
	{
		String id = null;
	...
	private static ModelicaModel buildModelicaModel(
			String staticId,
			List<BaseModelicaDeclaration> declarations,
			...
			List<AnnotationItem> annotations)
	{
		...
		if (annotations != null)
		{
			List<ModelicaInterconnection> interconnections = annotations.stream()
					.filter(a -> a.isPsmInterconnectionAnnotation())
					.map(a -> a.createInterconnection())
					.collect(Collectors.toList());
			m.setInterconnections(interconnections);
		}
