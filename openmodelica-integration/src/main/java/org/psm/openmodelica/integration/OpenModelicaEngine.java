...
public class OpenModelicaEngine implements ModelicaEngine
{
	...
	public void simulate(ModelicaDocument mo)
	{
		...
		try
		{
			...
			try
			{
				...
				if (result.err != null && !result.err.isEmpty())
				{
					...
					else throw new RuntimeException("Error setting the working directory "
				...
				try (DirectoryStream<Path> stream = Files.newDirectoryStream(omSimulationDir,
						"*.mo"))
				{
					for (Path p : stream)
					{
						...
						if (result.err != null && !result.err.isEmpty())
						{
							...
							else throw new RuntimeException("Error loading Modelica model: "
									+ p.toFile().getName() + ". " + result.err.replace("\"", ""));
						}
					}
				}
				catch (IOException e)
				...
				{
					if (result.err.contains("Warning")) LOGGER.warn(result.err.replace("\"", ""));
					else throw new RuntimeException("Error checking model " + modelName + ". {"
							+ result.err.replace("\"", "") + "}");
				}
				...
				while ((successful == false) && (i < METHOD_LIST.length))
				{
					LOGGER.info(
							...
							modelName, METHOD_LIST[i], startTime, stopTime, numOfIntervals,
							tolerance, simFlags);

					result = omc.simulate(modelName, startTime, stopTime, numOfIntervals,
							METHOD_LIST[i], tolerance, this.simFlags);
					if (result != null)
					{
						...
						{
							LOGGER.error(
									"Error simulating model {} with integration method {}. Reason is {}.",
									modelName, METHOD_LIST[i], simResult.getError());
							throw new RuntimeException("Error simulating model" + modelName
									+ " with integration method " + METHOD_LIST[i] + ". "
									+ simResult.getError());
						}
					...
					{
						LOGGER.error("Error simulating model {} with integration method {}.",
								modelName, METHOD_LIST[i]);
						throw new RuntimeException("Error simulating model" + modelName
								+ " with integration method " + METHOD_LIST[i] + ".");
					}
					i++;
				...
				if (result.err != null && !result.err.isEmpty())
				{
					if (result.err.contains("Warning:")) LOGGER.warn(result.err.replace("\"", ""));
							...
							"Error reading simulation results variables from " + matResultsFile
									+ ". " + result.err.replace("\"", ""));
				}
				...
				if (resultVariables != null && !resultVariables.isEmpty())
				{
					...
					filterResultVariables = allResultVariables.stream()
				...
				int resultSize = Integer.parseInt(
						omc.readSimulationResultSize(matResultsFile).res.replace("\n", ""));

				try (PrintStream printStream = new PrintStream(Files.newOutputStream(
						Paths.get(omSimulationDir + File.separator + csvResultsFile))))
				{
					writeResults(omc, printStream, matResultsFile, filterResultVariables,
							resultSize, createFilteredMat);
				}
				catch (IOException e)
				{
		...
		catch (Exception e)
		{
			...
			throw new RuntimeException("openmodelica simulation failed - remote working directory "
					+ workingDir + ", fileName: " + modelFileName + ", problem:" + modelName
	...
	public void simulate(Collection<ModelicaDocument> mos)
	{
		...
		// Temporal files are overwritten if run in parallel (equations not written properly)
	...
	private void writeResults(OpenModelicaWrapper omc, PrintStream printStream,
			String matResultsFile, List<String> filterResultVariables, int resultSize,
			boolean createFilteredMat) throws ConnectException
	{
		...
		for (int j = 0; j < filterResultVariables.size(); j++)
		{
			...
			if (result.err != null && !result.err.isEmpty())
			{
				...
				else LOGGER.error("Error getting simulation result variable {} from  {}. {}",
						resVar, matResultsFile, result.err.replace("\"", ""));
				continue;
			}
			...
			// LUMA resVar was quoted, remove quotes before storing in simulation results
		...
		try
		{
				...
				Files.list(this.libraryDir).forEach(file -> {
					...
					catch (IOException exc)
					{
						LOGGER.error("Error copying file from {} to {}, reason is {}",
								file.toFile(), this.omSimulationDir, exc.getMessage());
			...
			catch (Exception e1)
			{
				LOGGER.error("Error copying files from {} to {}, reason is {}", this.libraryDir,
						this.omSimulationDir, e1.getMessage());
			}
		}
		catch (IOException e)
		{
